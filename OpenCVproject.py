...
def highlightFace(net, frame, iii, schetchik, conf_threshold=0.7):
    ...
    #                   нагрузка на ЦП(Intel Core i5-7400 3.00 GHz 4core) 50%+-4% от Python`а
    # при conf_threshold=0.2 и рамке 400х400 видит лицо с расстояния 1.95 метра (при рамке 300х300 видит 1.75)
    #                   нагрузка на ЦП(Intel Core i5-7400 3.00 GHz 4core) 55%+-4% от Python`а
    # при conf_threshold=0.2 и рамке 700х700 видит лицо с расстояния 2+ метра(не измерял)
    #                   нагрузка на ЦП(Intel Core i5-7400 3.00 GHz 4core) 68%+-4% от Python`а
    ...
    for i in range(detections.shape[2]):
        ...
        # если результат превышает порог срабатывания, то это лицо
...
def doAFaceSearch():
    ...
    # получаем очередной кадр с камеры
        ...
        # выводим картинку с камеры(нужно для отладки или для вывода на экран компа)
        # cv2.imshow("zxc", resultImg)

        # Кодируем кадр в JPEG(в браузере обязательно д.б. JPEG или аналогичная передача)
    ...
    video.release()
# ДОЛЖЕН СТОЯТЬ В КОНЦЕ, ИНАЧЕ НИЧЕ НЕ ЗАРАБОТАЕТ!!!
